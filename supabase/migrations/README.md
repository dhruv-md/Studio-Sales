...
| `009_soft_delete.sql` | **Soft delete for projects and spaces.** `is_deleted boolean not null default false` on `studio_project` and `studio_project_space`, plus partial indexes for the live case. Reads filter it out (`lib/data/queries.ts`); delete sets the flag rather than removing rows. | ☑ applied 2026-09-19 via the Management API — probed: both `is_deleted` columns present, `boolean` default `false`. Existing ownership policies (007) already gate who can flip it; RLS suite still 221/221. |
| `008_phone_approval.sql` | **The linked-number approval gate.** `referral_phone.approval_status` (+ `approved_by`/`approved_at`), an insert trigger (`referral_phone_gate_ins`) that forces a partner-added number to `pending` (and the primary `client` number / an admin's own to `approved`), and `review_referral_phone(uuid,text)` — the one admin-checked way to move it. The sync and the live pull match only `approved` numbers. | ☑ applied 2026-09-18 via the Management API — probed: `approval_status` column present, trigger present, function present, all 15 existing rows grandfathered `approved`. **Drift to clean up:** a superseded parallel attempt (`status` column, `reviewed_*` columns, `referral_phone_guard` trigger, and a 3-arg `review_referral_phone(uuid,text,text)`) is also live in this project and should be dropped — nothing reads it. |
...
degrade.** `/api/sync/referrals` filters `referral_phone` on
`.eq('approval_status', 'approved')`, and a column that does not exist yet is a hard Postgres error,

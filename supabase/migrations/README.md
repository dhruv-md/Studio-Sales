...
| `../seed/001_demo.sql` | Demo data — a firm, 4 clients, 5 projects, boards, quotes, procurement, ledger, referrals, rewards | ☑ confirmed applied — probed 2026-09-17, every row count matches the file exactly (4 clients, 5 projects, 19 boards, 26 board products, 3 quotes, 3 referral orders) |
| `../seed/002_console.sql` | Demo console data — the team, two more firms, prospects, onboarding forms, portfolios, activity | ☐ confirmed NOT applied — probed 2026-09-17, `partner` has 1 row (Studio Terra only), not the 3 this file describes. Needs `supabase/seed/003_README.md`'s auth users first — see there. |
| `../seed/003_bulk_variety.sql` | Five more testable partner logins across every lifecycle state, mood boards, portfolio images, escalations, visit requests, team invites, notification prefs — see its own header | ☐ not yet applied — depends on 002 above. Auth users for both are documented and already provisioned, see `../seed/003_README.md` |

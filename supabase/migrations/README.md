...
| `008_phone_approval.sql` | **The linked-number approval gate.** `referral_phone.approval_status` (+ `approved_by`/`approved_at`), an insert trigger that forces a partner-added number to `pending` (and the primary `client` number / an admin's own to `approved`), and `review_referral_phone()` — the one admin-checked way to move it. The sync and the live pull match only `approved` numbers. | ☐ **NOT applied to production yet.** Paste via the Management API (the SQL Editor mangles long pastes — see CLAUDE.md), then probe: `referral_phone?select=approval_status` should be `200`, not `404`. Until it is applied, the app's `referral_phone` reads/writes of `approval_status` will 500. |
